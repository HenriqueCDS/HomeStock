-- V2: Optimized indexes for multi-tenant queries (MySQL)
-- MySQL does not support partial indexes (WHERE clause), so indexes cover all rows.
...
CREATE INDEX idx_products_tenant_ean   ON products(tenant_id, ean);
